...
const { getGoodById, editGood, loadGoods } = useGoods();
...
const loadGood = () => {
  const good = getGoodById(Number(route.params.id));
  ...
  form.value.name = good.name;
  form.value.description = good.description;
  form.value.category = good.category;
  form.value.price = Number(good.price);
  form.value.image = good.image;
  form.value.available = good.available;
};
...
const submitForm = () => {
  ...
  const result = editGood(Number(route.params.id), {
